...
function updateDashboard(data) {
    console.log('🔄 updateDashboard chamada', {
        hasConsumablesData: !!data.consumables_data,
...
function updateNumber(elementId, newValue) {
    ...
    console.log(`🔢 Atualizando ${elementId}: ${newValue}`);
...
// ========== FUNÇÕES DO SLIDE DE CONSUMÍVEIS ==========

/**
 * Atualizar slide de consumíveis/insumos
 */
function updateConsumablesSlide(data) {
    console.log('🔍 updateConsumablesSlide chamada');

    if (!data.consumables_data) {
        console.warn('⚠️ Dados de consumíveis não disponíveis');
        return;
    }

    const consumables = data.consumables_data;
    console.log('📦 Dados de consumíveis:', consumables.summary);

    // Atualizar cards de resumo
    updateNumber('total-consumable-items', consumables.summary.total_items);
    updateNumber('total-consumable-units', consumables.summary.available_units);
    updateNumber('low-stock-items', consumables.summary.low_stock_count);
    updateNumber('out-of-stock-items', consumables.summary.out_of_stock_count);

    // Atualizar tabela de alertas de estoque baixo
    updateLowStockTable(consumables.low_stock_alerts);

    // Atualizar gráfico de itens mais consumidos
    updateMostConsumedChart(consumables.most_consumed);

    console.log('✅ Slide de consumíveis atualizado');
}
...
function updateLowStockTable(alerts) {
    ...
    if (!alerts || alerts.length === 0) {
        tbody.innerHTML = '<tr><td colspan="4" style="text-align: center; color: #2ecc71;"><i class="fas fa-check-circle"></i> Todos os itens estão com estoque adequado</td></tr>';
        return;
    }

    let html = '';
    alerts.forEach(alert => {
        // Mapear status para classe CSS apropriada
        ...
        if (alert.status === 'critical') {
            ...
            statusClass = 'stock-info';
        }

        // Mapear status para ícone apropriado
        ...
        html += `
            <tr>
                <td>
                    ...
                    ${alert.ref ? `<br><small style="color: #95a5a6;">${alert.ref}</small>` : ''}
                </td>
                <td style="text-align: center;">${alert.available}</td>
                <td style="text-align: center;">${alert.threshold}</td>
                <td style="text-align: center;">
                    <span class="${statusClass}">
                        <i class="fas ${statusIcon}"></i> ${alert.status_label}
                    </span>
                </td>
            </tr>
        `;
 ...
 * Atualizar gráfico de itens mais consumidos
 */
function updateMostConsumedChart(items) {
    if (!items || items.length === 0) {
        return;
    }

    const ctx = document.getElementById('mostConsumedCanvas');
    if (!ctx) return;

    // Destruir gráfico anterior se existir
    if (window.mostConsumedChart) {
        window.mostConsumedChart.destroy();
    }
    ...
    const values = items.map(item => item.consumed_count);

    // Criar gráfico
    window.mostConsumedChart = new Chart(ctx, {
        type: 'bar',
        data: {
            labels: labels,
            datasets: [{
                ...
                tooltip: {
                    callbacks: {
                        title: function(context) {
                            ...
                            return items[context[0].dataIndex].name;
                        },
                        label: function(context) {
                            const item = items[context.dataIndex];
                            return [
                                `Consumidas: ${item.consumed_count} unidades`,
